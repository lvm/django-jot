;(function($){
    ...
    function update_objects_on_ct_change(cb){
        ...
        content_type.change(
            function(){
                var $get = $.get(window.__jot_get_objects_url__+$(this).val(),
                                 function(data){
                                     object_id.html(data);
                                 })
                    .done(function(){
                    ...
                    .fail(function(){
                        alert( window.__jot_ajax_error_msg__ );
                    });

                if( !window.__jot_cb_called__ && cb !== undefined &&
                    typeof(cb) == typeof(function(){}) ){
                    $get.done(cb);
                }
            });
    }
    ...
    function init(){
        if( window.__jot_child_object_id__ ){
            update_objects_on_ct_change(
                function(){
                    var object_id__options = $('#id_object_id')
                        .children('option');
                    object_id__options
                        .filter(':selected')
                        .removeAttr('selected');
                    $.each(object_id__options,
                           function(){
                               var $this = $(this);
                               if( $this.val() == window.__jot_child_object_id__){
